...
Elementar::Elementar(Grafo* g, float valorSub) : valorSubtrair(valorSub), menorCustoObtido(MAIS_INFINITO){
	ajuste = g->getNumSatellites();
	numCommodities = g->getNumCustomers();
...
int Elementar::calculaCaminhoElementar( Grafo* g ){
	...
	for (int i = 1; i <= numCommodities; ++i){
		...
		if (carga <= cargaMaxima){
			aux = new Label(verticesCoding[i], 1, i, 0, carga,	g->getCustoArestaDual(0, i), NULL);
			aux->pai = NULL;
			...
			if ((custoAteDestino < (valorSubtrair - 0.0001)) && (custoAteDestino < (menorCustoObtido-0.0001))){
	...
	while(menorIndice > 0){
		...
		for (int i = 1; i <= numCommodities; ++i){
			...
			if (((it->vertVisitados & verticesCoding[i]) == 0) && (carga <= cargaMaxima)){
				...
				if (!verificaLabelDominadoEDomina(i, custo, carga, numVisitados, conjVisitados)){
			...
			if ((vetorLabels[i].posAtual != NULL) && (vetorLabels[i].posAtual->custoDual < (menorCustoAtual-0.0001))){
				menorIndice = i;
...
bool Elementar::verificaLabelDominadoEDomina(int ult, float custo, int carga, int numVisit, unsigned long long int visit){
	...
	while (it != NULL){ //Verifica a dominancia para todos os labels, ou seja, 
		...
		if (custo < (it->custoDual-0.0001)){
			domCusto = 1;
		}else if (custo > (it->custoDual+0.0001)){
			domCusto = -1;
		}else{
			domCusto = 0;
		}
...
Rota** Elementar::getRotaCustoMinimo(Grafo* g, int satellite, float percentual){
	...
	if ( limiteAceitacao > (valorSubtrair - 0.0001 ) )
	{
		limiteAceitacao = valorSubtrair - 0.0001;
	}
		...
		for (int i = 1; i <= numCommodities; ++i){
			...
			while (aux != NULL){
				...
				if (custoR < limiteAceitacao){
					...
					r[numRotasRetornadas]->inserirVerticeFim(satellite);
					tmp = aux;
					while (tmp->pai != NULL){
					...
					r[numRotasRetornadas]->inserirVerticeInicio(tmp->ultimoVertice+ajuste);
					r[numRotasRetornadas]->inserirVerticeInicio(satellite);
...
void Elementar::imprimeLabel(ptrLabel aux){
	...
	printf("[%s'%d', %d(a), %d(u), %d(c), %0.02f($)]\n", bin, aux->numVertVisitados, 
			aux->verticeAntecessor, aux->ultimoVertice, aux->cargaAcumulada, aux->custoDual);				
	delete [] bin;
}
