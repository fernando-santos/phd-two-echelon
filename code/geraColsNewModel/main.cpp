...
int main(int argc, char** argv){
	...
	if ( ( sepCuts < 0 ) || ( sepCuts > 3 ) )
	{
		printf("A opção para separar cortes deve ser { 0, 1, 2, 3 }\n\n");
		exit(0);
	}
	...
	bool alcancouRaiz, bcpCap, bcpComb;
	...
	do
	{
		...
		bcpCap = ( ( mCplex->sepCuts == 1 ) || ( mCplex->sepCuts == 3 ) ) ? mCplex->addCapacityCuts(G, qRotas_no_temp, NULL) : false;
		if ( mCplex->sepCuts == 3 ) mCplex->solveMaster();
		bcpComb = ( ( mCplex->sepCuts == 2 ) || ( mCplex->sepCuts == 3 ) ) ? mCplex->addCombCuts(G, qRotas_no_temp, NULL) : false;

	}
	while ( bcpCap || bcpComb );
