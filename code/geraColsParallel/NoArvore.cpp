...
NoArvore::NoArvore(ptrNo pai, short int* arcoBranching){
	...
	alturaNaArvore = pai->alturaNaArvore + 1;
...
void NoArvore::atualizaCustosDuais( int s ){
	...
	for (int i = 0; i < arcosBranching.size(); ++i)
	{
		if ( arcosBranching[i][2] == s )
		{
			...
			custoDual = G->getCustoArestaDual(auxI, auxJ);
			custoDual -= mCplex->cplex.getDual(mCplex->constraintsArvoreL2[numRestGamma]);
			G->setCustoArestaDual(auxI, auxJ, custoDual);
		}
...
double NoArvore::runCplexInt()
{
	...
	for ( int s = 1; s <= nSatellites; ++s ) mCplex->gamma_noCGH[s].endElements();
	return limitePrimal;
}
...
void NoArvore::addNoProcessado(ptrNo no)
{
	...
	while ( ( it != NULL ) && ( no->alturaNaArvore < it->alturaNaArvore ) ) it = it->prox;
	aux = it->prox;
	it->prox = no;
	no->prox = aux;
}
...
/*
	...
	int numElementosMaiorAltura = 0;
	int maiorAltura = listaNosProcessados->alturaNaArvore;

	while ( ( it != NULL ) && ( it->alturaNaArvore == maiorAltura ) )
	{
		it = it->prox;
		++elementosMaiorAltura;
	}
*/
	ptrNo it = listaNosProcessados;
